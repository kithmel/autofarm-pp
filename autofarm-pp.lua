...
creditLbl=Create("TextLabel",main,{Position=UDim2.new(0,0,1,5),Size=UDim2.new(0,100,0,15),BackgroundTransparency=1,TextColor3=Color3.new(1,1,1),Text="by sirelKilla update by Nat ❤️",TextScaled=true,TextStrokeTransparency=.8})
...
while gui.Parent do
	...
	if doCashier then
		...
		if c then
			...
			if (root.Position-Vector3.new(46.34, 3.80, 82.02)).magnitude>9 then rootMoved = true smoothTP(CFrame.new(46.34, 3.80, 82.02)) wait(.1) end
	...
	if doCook then
		...
		if cookP and tick()-cookPtick>0.8 then
			...
			if oven==nil or oven.IsOpen.Value then
				cookPtick=tick()
				if (root.Position-Vector3.new(44.63, 6.60, 45.20)).magnitude>9 then rootMoved = true smoothTP(CFrame.new(44.63, 6.60, 45.20)) wait(.1) end
	...
	if doBoxer then
		...
		if boxD and tick()-boxDtick>0.8 then
			boxDtick=tick()
			if (root.Position-Vector3.new(54.09, 3.80, 23.150)).magnitude>9 then rootMoved = true smoothTP(CFrame.new(54.09, 3.80, 23.15)) wait(.1) end
		...
		if refill then
			...
			while doSupplier do
				...
				for _,btn in ipairs(supplyButtons) do
					if supplyCounts[bcolorToSupply[btn.BrickColor.Name]] < settings.refill_end then
						local count = 0
						if #boxes > 30 then
							for i=1,#boxes do
								local box = boxes[i]
						...
						if count < 2 then
							if fulfilled then
								root.CFrame = btn.CFrame + Vector3.new(0,3,3)
								wait(0.1)
							end
